...
from models.qat_yolo import Model
from models.yolo import Model as Exact_Model
...
def train(hyp, opt, device, callbacks):
    ...
    if pretrained:
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get("anchors"), mode=opt.mode).to(
            device
        )  # create
    ...
    if opt.kd:
        ...
        teacher_model = Exact_Model(
            cfg_teacher or teacher_ckpt["model"].yaml,
            ch=3,
            nc=nc,
            anchors=hyp.get("anchors"),
        ).to(
        ...
        exclude = (
            ["anchor"] if (cfg_teacher or hyp.get("anchors")) and not resume else []
        )  # exclude keys
        csd = (
            teacher_ckpt["model"].float().state_dict()
        )  # checkpoint state_dict as FP32
        csd = intersect_dicts(
            csd, teacher_model.state_dict(), exclude=exclude
        )  # intersect
    ...
    relabel_path = opt.relabel_path
    ckpts_list = [os.path.join(ckpt_path, item) for item in os.listdir(ckpt_path)]
    if relabel_path:
        relabels_list = [
            os.path.join(relabel_path, item) for item in os.listdir(relabel_path)
        ]
        assert len(relabels_list) == len(
            ckpts_list
        ), f"Len(relable list) is {len(relabels_list)}, Len(ckpt list) is {len(ckpts_list)}"
    if opt.calibration_size != 0:
        ckpt = torch.load(ckpts_list[0], map_location="cpu")
        bs = ckpt[opt.train_kind].shape[0]
        sublen = int(opt.calibration_size / bs) + 1
        ckpts_list = ckpts_list[:sublen]
        if relabel_path:
            relabels_list = relabels_list[:sublen]
        print(
            f"calibration size is {opt.calibration_size}, batch size is {bs}, thus select first {sublen} batches"
        )
    ...
    # Start training
    t0 = time.time()
    nb = len(ckpts_list)  # number of batches
    nw = max(
    ...
    if WORLD_SIZE > 1:
        model.module.start_quantize()
        add_name(model.module)
    else:
        model.start_quantize()
        add_name(model)
    ...
    for epoch in range(
        ...
        if opt.kd:
            teacher_model.eval()  # we need to set teacher to train mode to enable output align with student model, setting requires_grad=False
        ...
        optimizer.zero_grad()
        Loss, Ori_Loss, MSE_Loss, Distill_Loss = (
            np.array([]),
            np.array([]),
            np.array([]),
            np.array([]),
        ...
        for (
            ...
            imgs, paths = ckpts[opt.train_kind], ckpts["path"]
            # test effect of random gaussian data
            # imgs = torch.randint(high=255, size=imgs.size()).to(device, non_blocking=True).float() / 255
            if not relabel_path:
                targets = ckpts["target"]
            else:
                labels = torch.load(relabels_list[i], map_location=device)
            ...
            with torch.cuda.amp.autocast(amp):
                # loss
                if opt.kd:
                    # loss, ori_loss, KD_loss, MSE_loss, loss_items = kd_loss(imgs, targets.to(device))
                    loss, loss_items = kd_loss(imgs, targets.to(device))
                    # Loss = np.append(Loss, loss.item())
                    # Ori_Loss = np.append(Ori_Loss, ori_loss.item())
                    # Distill_Loss = np.append(Distill_Loss, KD_loss.item())
                    # MSE_Loss = np.append(MSE_Loss, MSE_loss.item())
                    # print(f'mse: {MSE_loss * opt.mse_loss_weight}, ori: {ori_loss*opt.ori_loss_weight}, distill: {KD_loss*opt.kd_loss_weight}')
                    # Box_Loss = np.append(Box_Loss, loss_items[0].item())
                    # Obj_Loss = np.append(Obj_Loss, loss_items[1].item())
                    # Cls_loss = np.append(Cls_loss, loss_items[2].item())
                else:
            ...
            # check PTQ result
            if opt.check_ptq and epoch == 0 and i == 0:
                if RANK in {-1, 0}:
                    if ema:
                        ema.update(model)
                    ema.update_attr(
                    ...
                    result_dict = {"map": results[3], "map50": results[2]}
                    with open(f"{save_dir}/ptq_results.json", "w") as fout:
        ...
        # record avg metric
        # if opt.kd:
        #     loss_record[epoch] = {'loss': np.mean(Loss).item(), 'ori_loss': np.mean(Ori_Loss).item(),
        #                         'kd_loss': np.mean(Distill_Loss).item(), 'mse_Loss': np.mean(MSE_loss).item()}
        #     print(loss_record[epoch])
        # Scheduler
        ...
        # end epoch ----------------------------------------------------------------------------------------------------
    if opt.kd:
        with open(f"{save_dir}/loss_gpu{LOCAL_RANK}.json", "w") as fout:
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--teacher-weight", type=str, default="", help="teacher model name"
    )
    parser.add_argument("--teacher-cfg", type=str, default="", help="model.yaml path")
    parser.add_argument(
    ...
    parser.add_argument(
        "--relabel_path", type=str, default=None, help="path to new data label"
    )
    parser.add_argument(
        "--train_kind",
        type=str,
    ...
    # Sparsity ratio
    parser.add_argument(
        "--sparsity-ratio", type=float, default=0.1, help="Sparsity Ratio"
    )
    parser.add_argument(
        "--prune-mode",
        type=str,
        default="output",
        help="how to use wanda to prune the model",
    )
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt.data, opt.cfg, opt.teacher_cfg, opt.hyp, opt.weights, opt.project = (
            check_file(opt.data),
            check_yaml(opt.cfg),
            check_yaml(opt.teacher_cfg),
            check_yaml(opt.hyp),
            str(opt.weights),
            str(opt.project),
        )  # checks
