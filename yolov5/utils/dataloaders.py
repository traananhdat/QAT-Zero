...
class SmartDistributedSampler(distributed.DistributedSampler):
    def __iter__(self):
        ...
        n = (
            int((len(self.dataset) - self.rank - 1) / self.num_replicas) + 1
...
def create_dataloader(
    ...
    seed=0,
    calibration_size=0,
    max_target=None,  # if set to a integer, pics with more than threshold targets will be ignored
    min_target=None,
):
    if rect and shuffle:
        LOGGER.warning(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(
            ...
            prefix=prefix,
            rank=rank,
            max_items=max_target,
            min_items=min_target,
        )
    ...
    if calibration_size != 0:
        print(f"before calibration: dataset len is {len(dataset)}")
        labels = dataset.labels
        dataset = Subset(dataset, range(calibration_size))
        dataset.labels = labels
        print(f"after calibration: dataset len is {len(dataset)}")
    return (
        loader(
            dataset,
...
class LoadStreams:
    ...
    def __next__(self):
        ...
        if self.transforms:
            ...
            im = np.stack(
                [
                    letterbox(x, self.img_size, stride=self.stride, auto=self.auto)[0]
                    for x in im0
                ]
            )  # resize
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        if max_items:
            include = (
                np.array([len(x) <= max_items for x in self.labels])
                .nonzero()[0]
                .astype(int)
            )
            LOGGER.info(f"{prefix}{n - len(include)}/{n} images filtered from dataset")
        ...
        if rank > -1:  # DDP indices (see: SmartDistributedSampler)
            ...
            self.indices = self.indices[
                np.random.RandomState(seed=seed).permutation(n) % WORLD_SIZE == RANK
            ]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp["mixup"]:
                img, labels = mixup(
                    img, labels, *self.load_mosaic(random.choice(self.indices))
                )
...
def extract_boxes(path=DATASETS_DIR / "coco128"):
    ...
    (
        shutil.rmtree(path / "classification")
        if (path / "classification").is_dir()
        else None
    )  # remove existing
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, i):
        ...
        if self.album_transforms:
            sample = self.album_transforms(image=cv2.cvtColor(im, cv2.COLOR_BGR2RGB))[
                "image"
            ]
        else:
