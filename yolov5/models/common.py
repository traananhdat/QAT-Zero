...
from .qatops import LSQconv2d
from .ptqops import Qconv2d
from .lsqops import QuantConv2d
from .wandaops import WandaConv2d
from .lsqops2 import LSQConv
from .lsqplus import QuantConv2dPlus
from .lsqplus2 import QuantConv2dPlus2
...
class Conv(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if num_bits == 32 or mode == "exact":
            self.conv = nn.Conv2d(
                c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False
            )
        elif mode.startswith("quantize_sym"):
            self.conv = LSQconv2d(
                c1,
                c2,
                k,
            ...
            # self.conv = QuantConv2dPlus(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False, a_bits=num_bits, w_bits=num_bits, mode=mode)

            # self.conv = QuantConv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False, a_bits=num_bits, w_bits=num_bits, initialize_mode=mode)
            # self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        # self.conv = Qconv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False, num_bits=num_bits, mode=mode)
        # self.conv = QuantConv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False, a_bits=num_bits, w_bits=num_bits)
        # self.conv = WandaConv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False, sparsity_ratio=0.1)
        ...
        # self.drop = None
        # drop_list = ['droptotal', 'dropcnn']
        # for text in drop_list:
        #     if text in mode:
        #         drop = float(mode.split(text)[1])
        #         self.drop = nn.Dropout2d(drop)

    def start_quantize(self):
        if not (type(self.conv) == nn.Conv2d):
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        """
        super().__init__()
        print("There should not be TransformerBlock in yolo model!")
        exit(0)
        self.conv = None
        if c1 != c2:
...
class Bottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, num_bits=8, mode="exact"):
        """Initializes a standard bottleneck layer with optional shortcut and group convolution, supporting channel
        expansion.
        """
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1, num_bits=num_bits, mode=mode)
        self.cv2 = Conv(c_, c2, 3, 1, g=g, num_bits=num_bits, mode=mode)
...
class C3(nn.Module):
    ...
    def __init__(
        self, c1, c2, n=1, shortcut=True, g=1, e=0.5, num_bits=8, mode="exact"
    ):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1, num_bits=num_bits, mode=mode)
        self.cv2 = Conv(c1, c_, 1, 1, num_bits=num_bits, mode=mode)
        self.cv3 = Conv(
            2 * c_, c2, 1, num_bits=num_bits, mode=mode
        )  # optional act=FReLU(c2)
        self.m = nn.Sequential(
            *(
                Bottleneck(c_, c_, shortcut, g, e=1.0, num_bits=num_bits, mode=mode)
                for _ in range(n)
            )
        )
    ...
    def start_prune(self, sparsity=None, prune_mode=None):
        ...
        for model in self.m:
            if isinstance(model, Bottleneck):
                model.start_prune(sparsity, prune_mode)
...
class SPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5, num_bits=8, mode="exact"):
        """
        ...
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1, num_bits=num_bits, mode=mode)
        self.cv2 = Conv(c_ * 4, c2, 1, 1, num_bits=num_bits, mode=mode)
