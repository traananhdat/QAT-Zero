...
from external.dist import ddp_cleanup, generate_ddp_command
from quant.kd_loss import KDLoss
from quant.quantize import inplace_quantize, converted_modules
...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.epochs = (
            self.args.epochs or 100
        )  # in case users accidentally pass epochs=None with timed training
        self.start_epoch = 0
        if RANK == -1:
    ...
    def train(self, quant_mode=None):
        ...
        if world_size > 1 and "LOCAL_RANK" not in os.environ:
            ...
            cmd, file = generate_ddp_command(
                world_size, self, quant_mode, self.kd_method, self.calibration
            )
            try:
            ...
            if quant_mode:  # this hack is for distributed
                self.set_quantize(quant_mode)
    ...
    def _setup_train(self, world_size):
        ...
        if RANK in {-1, 0}:
            ...
            if self.args.plots and not self.use_generated:
    ...
    def _do_train(self, world_size=1):
        ...
        # delayed here to avoid dry run
        if self.kd_method:
            self.kd_loss.enable_kd()
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                def kd_adapter(batch):
                    preds = self.model(batch["img"])
                    return self.kd_loss(batch, preds)

                # Forward
                with autocast(self.amp):
                    batch = self.preprocess_batch(batch)
                    self.loss, self.loss_items = (
                        kd_adapter(batch) if self.kd_loss else self.model(batch)
                    )
                    if RANK != -1:
                ...
                if self.ptq:
                    ...
                    ptq_file = self.save_dir / "ptq.json"
                    with ptq_file.open("w") as f:
                        f.write(str(metrics))
                    print(metrics)
                    import sys
                ...
                if RANK in {-1, 0}:
                    ...
                    if (
                        self.args.plots
                        and ni in self.plot_idx
                        and not self.use_generated
                    ):
    ...
    def setup_model(self):
        ...
        if self.quantize:
            self.model = inplace_quantize(self.model, self.quantize_mode)

            for m in self.model.model:
                if isinstance(m, converted_modules):
                    m.start_quantize()
        ...
        if self.kd_method:
            ...
            weights, _ = attempt_load_one_weight(self.kd_method.teacher_weight)
            cfg = weights.yaml
            teacher_model = self.get_model(
                cfg=cfg, weights=weights, verbose=RANK == -1
            ).to(self.device)
            kd_loss = KDLoss(
                student=self.model,
                teacher=teacher_model,
                ori_loss=self.model.loss,
                kd_methods=self.kd_method,
                device=self.device,
            )
    ...
    def save_metrics(self, metrics):
        ...
        s = (
            ""
            if self.csv.exists()
            else (("%s," * n % tuple(["epoch", "time"] + keys)).rstrip(",") + "\n")
        )  # header
        t = time.time() - self.train_time_start
        with open(self.csv, "a") as f:
            f.write(
                s + ("%.6g," * n % tuple([self.epoch + 1, t] + vals)).rstrip(",") + "\n"
            )
