...
__all__ = ["swin_tiny_patch4_window7_224", "swin_small_patch4_window7_224"]
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = QuantLinear(cfg.weight_bit, dim, dim * 3)
        self.qact1 = QuantAct(cfg.activation_bit)
        self.qact_attn1 = QuantAct(cfg.activation_bit)
        self.qact_table = QuantAct(cfg.activation_bit)
        ...
        self.proj = QuantLinear(cfg.weight_bit, dim, dim)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        x = self.qkv(x)
        x = self.qact1(x)
        qkv = x.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(
            2, 0, 3, 1, 4
        )
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, cfg=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        ...
        self.reduction = QuantLinear(cfg.weight_bit, 4 * dim, 2 * dim, bias=False)
        self.qact2 = QuantAct(cfg.activation_bit)
    ...
    def __init__(
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_resolution, dim=dim, norm_layer=norm_layer, cfg=cfg
            )
        else:
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.qact3 = QuantAct(cfg.activation_bit)
        self.head = QuantLinear(cfg.weight_bit, self.num_features, num_classes)
...
def swin_tiny_patch4_window7_224(pretrained=False, cfg=None, **kwargs):
