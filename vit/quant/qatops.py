...
ALL_MODES = ["exact", "quantize_sym", "quantize_asym"]
...
class LSQconv2d(nn.Conv2d):
    ...
    def forward(self, input):
        if not self.quantize:
            print("You haven't enable quantizer in a quantized module")
            print("This might be a bug, please ignore this notice if intended.")
            raise RuntimeError()
    ...
    def __repr__(self):
        s = super().__repr__()
        s = f"(layer={s}, qmode={self.mode}, wbit={self.weight_bits}, abit={self.activation_bits}, enabled={self.quantize})"
        return s
...
class LSQLinear(nn.Linear):
    ...
    def __init__(
        ...
        assert mode in ALL_MODES, f"input mode {mode}, all valid options: {ALL_MODES}"
        self.mode = mode
        self.weight_bits = weight_bits
        self.activation_bits = activation_bits
        self.scale_input = nn.Parameter(torch.tensor(1.0), requires_grad=True)
    ...
    def __repr__(self):
        s = super().__repr__()
        s = f"(layer={s}, qmode={self.mode}, wbit={self.weight_bits}, abit={self.activation_bits}, enabled={self.quantizer_enabled})"
        return s
    ...
    def start_quantize(self):
        ...
        print(f"enabled quantize for layer {self}")

    def init_quantize(self, input: torch.Tensor):
        Qsym_w = 2 ** (self.weight_bits - 1) - 1
        Qsym_a = 2 ** (self.activation_bits - 1) - 1
        Qasym_a = 2**self.activation_bits - 1
        self.scale_weight.data.copy_(
    ...
    def forward(self, input: torch.Tensor):
        if not self.quantizer_enabled:
            print("You haven't enable quantizer in a quantized module")
            print("This might be a bug, please ignore this notice if intended.")
            raise RuntimeError()
        if self.quantizer_enabled:
            if not self.quantizer_initialized:
                self.init_quantize(input=input)
            else:
                self.scale_input.data = self.scale_input.data.abs()
                self.scale_weight.data = self.scale_weight.data.abs()
        ...
        def quantize_input(input):
            if self.mode == "exact":
                return input
            elif self.mode == "quantize_sym":
                return SymLsqQuantizer.apply(
                    input, self.scale_input, self.activation_bits
                )
            else:
                return AsymLsqQuantizer.apply(input, self.scale_input, self.weight_bits)
        ...
        def quantize_weight(weight):
            if self.mode == "exact":
                ...
                return SymLsqQuantizer.apply(
                    weight, self.scale_weight, self.weight_bits
                )

        return F.linear(
            input=quantize_input(input=input),
            weight=quantize_weight(weight=self.weight),
            bias=self.bias,
        )
